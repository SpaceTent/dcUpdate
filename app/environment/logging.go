	...
	handler := l.New(
		tint.NewHandler(os.Stdout, &tint.Options{
			...
			ReplaceAttr: func(groups []string, attr l.Attr) l.Attr {
				...
				if attr.Key == "source" {
					source_file := strings.Replace(attr.Value.String(), CURRENT_DIR, "", -1)
					newline := strings.Split(source_file, " ")
					Namespace := strings.Split(newline[0], "/")

					out := Namespace[len(Namespace)-1] + " " + newline[1] + " " + newline[2]
